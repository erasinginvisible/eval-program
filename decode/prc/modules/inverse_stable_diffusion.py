...
from modules.modified_stable_diffusion import ModifiedStableDiffusionPipeline
...
class InversableStableDiffusionPipeline(ModifiedStableDiffusionPipeline):
    ...
    def decoder_inv(self, x):
        ...
        for n in self.progress_bar(range(20)):  # Only 20 steps
            #print(n)
            # x_pred = self.decode_image_for_gradient_float(z)
            scaled_latents = 1 / 0.18215 * z
            #vae = copy.deepcopy(self.vae).float()
            # image = [
            #     self.vae.decode(scaled_latents[i : i + 1]).sample for i in range(len(z))
            # ]
            image = []
            for i in range(len(z)):
                #print(f"i: {i}")
                image += self.vae.decode(scaled_latents[i : i + 1]).sample
