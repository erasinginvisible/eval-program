...
from src.inverse_vae import InversableVAE
from src.inverse_stable_diffusion import InversableStableDiffusionPipeline

warnings.filterwarnings("ignore")

MODEL_ID = "stabilityai/stable-diffusion-2-1-base"
CHECKPOINT_PATH = "./model/prc.pt"
CACHE_DIR = "/tmp/bang/.cache/huggingface/"  # "./cache/"
...
def exact_inversion_vae(
    ...
    # image to latent
    images_tensor = (
        torch.stack([transform_img(img) for img in images])
        .to(torch.float32)
        .to(device)
    )
...
def load_vae(checkpoint_path=CHECKPOINT_PATH, device="cuda"):
    vae = InversableVAE(
...
def load_pipe(model_id=MODEL_ID, cache_dir=CACHE_DIR, solver_order=1):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    scheduler = DPMSolverMultistepScheduler(
        beta_end=0.012,
        beta_schedule="scaled_linear",
...
def decode_batch_rest(
    ...
    text_embeddings_tuple = pipe.encode_prompt(
        ["",]*batch_size, "cuda", 1, guidance_scale > 1.0, None
    )
    ...
    for idx in tqdm(np.arange(0, len(image_latents), batch_size)):
        reversed_latent_batch = pipe.forward_diffusion(
            latents=torch.stack(image_latents[idx:min(idx+batch_size, len(image_latents))]),
