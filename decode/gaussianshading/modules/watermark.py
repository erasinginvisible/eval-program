...
class Gaussian_Shading_chacha:
    def __init__(self, ch_factor, hw_factor, fpr, user_number, watermark):
        self.ch = ch_factor
        self.hw = hw_factor
        self.nonce = None
        self.key = None
        self.watermark = watermark.cuda()
        self.latentlength = 4 * 64 * 64
        self.marklength = self.latentlength // (self.ch * self.hw * self.hw)

        self.threshold = (
            1 if self.hw == 1 and self.ch == 1 else self.ch * self.hw * self.hw // 2
        )
        ...
        self.key = b"\xaf\x14\xdd\xb8nw.8\n\x1d\x89v\x01\xc1j\xce\xe1\xda\xd6(<\x95\x06\xeb\xc8\xa3>\x06\x17|\xad1"
        self.nonce = b"\xb8\x1f\x98\xf1 \xbd\xdff\xeb\xd6\x97_"
    ...
    def eval_watermark(self, reversed_w):
        ...
        return reversed_watermark, correct
